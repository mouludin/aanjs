...
Aan.prototype.update = function(cb){
    cb(this.data);
    this.action();
};
...
Aan.prototype.draw = function(para){
    ...
    let new_data_process = () => {
        ...
        if(ctxh > wh * 0.8){y_min < 0? ny = 11:ny = 10;}
        else if(ctxh > wh * 0.4 && ctxh <= wh * 0.8){ny = 5}
        else if(ctxh > wh * 0.2 && ctxh <= wh * 0.4){y_min < 0?  ny = 3:  ny = 4};
        if(y_min < 0){ny -= 1};
    };
                    ...
                    for(let n = 0; n < this.data.Yaxis.length;n++){
                        ...
                        if(Xaxiss.length > 0){
                            ...
                            if(this.data.Yaxis[n].data[j]){
                                let y = y_for_rect + (20 * data_index_j.length * as) + 15;
    ...
    this.action();
    canvas.addEventListener('mousemove',()=>{this.action()});
};
