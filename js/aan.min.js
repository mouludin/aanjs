...
ym=Math.max(...ayd);ymi=Math.min(...ayd);nny=doy(ymi,ym)[1];sY=doy(ymi,ym)[0];Ya=doy(ymi,ym)[2];if(ch>wh*0.8){ny=10}else if(ch>wh*0.4&&ch<=wh*0.8){ny=5}else if(ch>wh*0.2&&ch<=wh*0.4){ny=4};if(ymi<0){ny-=1}};ndp();let ci=(x,y,color,width)=>{c.fillStyle=color;c.beginPath();c.arc(x,y,width,0,Math.PI*2,true);c.fill();};let re=(x,y,w,h)=>{c.beginPath();c.fillStyle="rgba(220,220,220,0.5)";c.rect(x,y,w,h);c.fill()};
